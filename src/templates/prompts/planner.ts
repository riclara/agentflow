...
export function renderPlannerPrompt(config: AgentflowConfig, variant: "markdown" | "codex" | "opencode" = "markdown"): string {
  if (variant === "opencode") {
...
---
Plan written to FEATURE_DIR/plan.md. Review it and when ready run:

@implementer Read FEATURE_DIR/plan.md and execute only ## Implementation Tasks.
...
After writing the review, end your response with one of:

If APPROVED:
---
Review written to FEATURE_DIR/review.md — Status: APPROVED. Run:

@tester Read FEATURE_DIR/plan.md and execute only ## Test Tasks.

If NEEDS_CHANGES:
---
Review written to FEATURE_DIR/review.md — Status: NEEDS_CHANGES. Run:

@implementer Read FEATURE_DIR/review.md and fix every issue listed under ## Issues.
